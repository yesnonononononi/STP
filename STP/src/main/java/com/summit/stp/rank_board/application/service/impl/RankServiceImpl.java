...
public class RankServiceImpl implements RankService {
    ...
    public Result<List<CreatorRankVO>> queryCreatorRankList(Integer size, Integer status) {
        List<RankBoard> resDb = repository.queryCreatorRank(size);
        List<Long> list = resDb.stream().map(RankBoard::getEntityId).toList();
        Map<Long, UserSimpleVO> uMap = userFiller.fillUsers(list);
        List<CreatorRankVO> res = resDb.stream().map(rb -> this.toCreatorVO(rb, uMap.get(rb.getEntityId()))).toList();
