...
public class RankUpdateScheduler {
    ...
    public void execute() {
        distributedLockUtil.executeWithLock(RedisConstants.Rank.LOCK, this::conduct);
    }
    ...
    private void updateCreatorRankBoard(List<Post> list) {
        List<RankBoard> res = list.stream()
                .map(post -> toDomain(post.getCreatorId(), null, post.getHotScore() != null ? post.getHotScore() : 0.0, 0, 1, BoardType.CREATOR.getType()))
