...
import com.summit.stp.comment.domain.util.ScoreUtil;
...
public class Post {
    ...
    public void updateScope(Integer visible) {
        VisibleScope scope = VisibleScope.fromCode(visible);
    ...
    public static boolean isLimited(Serializable imageUrlCount) {
        return ((Number) imageUrlCount).intValue() > BusinessRuleConstants.Post.MAX_IMAGE_NUM;
    }


    public void updatePost(Post post){
        if (post.title != null && post.title.length() > BusinessRuleConstants.Post.MAX_TITLE_LENGTH) {
            throw new IllegalArgumentException("帖子标题长度不能超过" + BusinessRuleConstants.Post.MAX_TITLE_LENGTH + "字");
        }
        if (post.content != null && post.content.length() > BusinessRuleConstants.Post.MAX_CONTENT_LENGTH) {
            throw new IllegalArgumentException("帖子内容长度不能超过" + BusinessRuleConstants.Post.MAX_CONTENT_LENGTH + "字");
        }
