    ...
    public void onEvent(UserLikedChangeEvent event, Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            if (rows == 0) {
                ...
                UserStatPO po = UserStatPO.builder()
                        .userId(userId)
                        .fans(0L)
                        .topic(0L)
                        .liked(Math.max(0L, delta.longValue())) // 防止负数越界
                        .build();
                try {
                    userStatMapper.insert(po);
                    log.info("【用户获赞统计MQ监听】已为用户创建初始统计档案，并初始化 liked: userId={}, liked={}", userId, po.getLiked());
    ...
    public void onFansEvent(UserFansChangeEvent event, Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            if (rows == 0) {
                ...
                UserStatPO po = UserStatPO.builder()
                        .userId(userId)
                        .fans(Math.max(0L, delta.longValue())) // 防止负数越界
                        .topic(0L)
                        .liked(0L)
                        .build();
                try {
                    userStatMapper.insert(po);
                    log.info("【用户粉丝统计MQ监听】已为用户创建初始统计档案，并初始化 fans: userId={}, fans={}", userId, po.getFans());
                } catch (DuplicateKeyException e) {
                    // 3. 并发冲突重试
