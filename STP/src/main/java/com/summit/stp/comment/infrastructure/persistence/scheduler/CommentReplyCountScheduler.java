...
public class CommentReplyCountScheduler {
    ...
    public void updateCommentReplyCount() {
        ...
        distributedLockUtil.executeWithLock(RedisConstants.Comment.REPLY_CHANGED_LOCK, this::conduct);
    }

    private void conduct() {
        // Tip: 1. 扫描并优先处理断电等突发状况下遗留下来的临时备份 Key
        Set<Object> leftoverKeys = redisTemplate.keys(RedisConstants.Comment.REPLY_CHANGED_RUN_PREFIX + "*");
        ...
        String originalKey = RedisConstants.Comment.REPLY_CHANGED;
        if (Boolean.TRUE.equals(redisTemplate.hasKey(originalKey))) {
            String backupKey = RedisConstants.Comment.REPLY_CHANGED_RUN_PREFIX + UUID.randomUUID();
            try {
