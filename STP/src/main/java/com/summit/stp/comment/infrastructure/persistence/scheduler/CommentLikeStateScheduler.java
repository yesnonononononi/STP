...
public class CommentLikeStateScheduler {
    ...
    public void updateCommentLikeState() {
        ...
        distributedLockUtil.executeWithLock(RedisConstants.Comment.CHANGED_LOCK, this::conduct);
    }

    private void conduct() {
        // Tip: 1. 扫描崩溃遗留下来的备份 Key，优先处理未完任务（断点续传）
        Set<Object> leftoverKeys = redisTemplate.keys(RedisConstants.Comment.CHANGED_RUN_PREFIX + "*");
        ...
        String originalKey = RedisConstants.Comment.CHANGED;
        if (Boolean.TRUE.equals(redisTemplate.hasKey(originalKey))) {
            String backupKey = RedisConstants.Comment.CHANGED_RUN_PREFIX + UUID.randomUUID();
            try {
