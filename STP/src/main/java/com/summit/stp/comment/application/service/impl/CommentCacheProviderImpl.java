...
public class CommentCacheProviderImpl implements CommentCacheProvider {
    ...
    public Map<Long, Map<String, Long>> getLikeCountAndReplyCount(List<Long> list) {
        ...
        List<Object> resList = redisTemplate.executePipelined(new SessionCallback<>() {
            ...
            public Object execute(@NonNull RedisOperations operations) throws DataAccessException {
                ...
                for (Long id : list) {
                    ...
                    setOps.size(buildKey(RedisConstants.Comment.LIKE, id));
                    // Tip: 获取回复数 String 缓存中的值
                    valOps.get(buildKey(RedisConstants.Comment.REPLY_COUNT, id));
                }
                return null;
            }
        });
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            String likeKey = buildKey(RedisConstants.Comment.LIKE, commentId);
    ...
    public Map<Long, Map<String, Boolean>> getIsLikeAndReplyByIds(List<CommentVO> list) {
        ...
        // Tip: 获取当前登录人 ID，如未登录则传游客 0L 占位
        Long currentUserId = (UserHolder.getUser() != null) ? UserHolder.getUser().getId() : 0L;
        ...
        List<Object> result = redisTemplate.executePipelined(new SessionCallback<>() {
            ...
            public Object execute(@NonNull RedisOperations operations) throws DataAccessException {
                ...
                for (int i = 0; i < ids.size(); i++) {
                    ...
                    String userIsLikeKey = (String) buildKey(RedisConstants.Comment.LIKE, commentId);
                    String isReplyKey = (String) buildKey(RedisConstants.Comment.REPLY_COUNT, commentId);
    ...
    public void init(Long commentId, Long replyCount, Long likeCount) {
        ...
        redisTemplate.executePipelined(new SessionCallback<>(){
            ...
            public Object execute(RedisOperations operations) throws DataAccessException {
                ...
                redisTemplate.expire(likeKey, RedisConstants.Comment.EXPIRE_TIME, TimeUnit.DAYS);

                // Tip: 回复数 String 初始化为传入的计数值
                String replyKey = buildKey(RedisConstants.Comment.REPLY_COUNT, commentId);
                operations.opsForValue().set(replyKey, replyCount != null ? replyCount : 0L);
                redisTemplate.expire(replyKey, RedisConstants.Comment.EXPIRE_TIME, TimeUnit.DAYS);
                return null;
            }
        });
    }
    ...
    public boolean like(Long commentId) {
        ...
        boolean isOK;
        String key = buildKey(RedisConstants.Comment.LIKE, commentId);
    ...
    public void reply(Long commentId) {
        ...
        String key = buildKey(RedisConstants.Comment.REPLY_COUNT, commentId);
    ...
    public void markCacheChanged(Long commentId) {
        ...
        redisTemplate.opsForSet().add(RedisConstants.Comment.CHANGED, commentId);
    }
    ...
    public void markReplyChanged(Long commentId) {
        ...
        if (commentId != null) {
            redisTemplate.opsForSet().add(RedisConstants.Comment.REPLY_CHANGED, commentId);
        }
    }
    ...
    public void loadCache(List<Long> list) {
        ...
        List<Object> existsResults = redisTemplate.executePipelined(new SessionCallback<>() {
            ...
            public Object execute(@NonNull RedisOperations operations) throws DataAccessException {
                for (Long id : list) {
                    operations.hasKey(buildKey(RedisConstants.Comment.LIKE, id));
                    operations.hasKey(buildKey(RedisConstants.Comment.REPLY_COUNT, id));
                }
                return null;
            }
        });
            ...
            redisTemplate.executePipelined(new SessionCallback<>() {
                ...
                public Object execute(@NonNull RedisOperations operations) throws DataAccessException {
                    ...
                    for (Long id : missingLikeIds) {
                        String key = buildKey(RedisConstants.Comment.LIKE, id);
                        // Tip: 写入 -1L 防缓存击穿占位符
                        setOps.add(key, -1L);
                        ...
                        operations.expire(key, RedisConstants.Comment.EXPIRE_TIME, TimeUnit.DAYS);
                    }

                    // Tip: 回源预热回复数 String 计数值
                    for (Long id : missingReplyIds) {
                        String key = buildKey(RedisConstants.Comment.REPLY_COUNT, id);
