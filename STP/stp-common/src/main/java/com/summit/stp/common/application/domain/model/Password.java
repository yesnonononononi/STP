...
public class Password {
    ...
    private static void validate(String value) {
        ...
        if (value.length() < UserAuthConstants.Business.MIN_PASSWORD_LENGTH) {
            throw new ParameterException(
                String.format("密码长度不能小于%d", UserAuthConstants.Business.MIN_PASSWORD_LENGTH));
        }
        if (value.length() > UserAuthConstants.Business.MAX_PASSWORD_LENGTH) {
            throw new ParameterException(
                String.format("密码长度不能大于%d", UserAuthConstants.Business.MAX_PASSWORD_LENGTH));
        }
