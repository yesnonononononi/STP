...
public class RedisOrderTimeoutProvider implements OrderTimeoutProvider {
    ...
    public void registerTimeout(Long duration, Long orderId) {
        ...
        zset.add(OrderConstants.Cache.TIMEOUT_ZSET, String.valueOf(orderId), (double) timeout.getTime());
    }
    ...
    public void cancelTimeout(Long orderId) {
        try {
            ZSetOperations<String, String> zset = stringRedisTemplate.opsForZSet();
            zset.remove(OrderConstants.Cache.TIMEOUT_ZSET, String.valueOf(orderId));
        }catch (Exception e){
            log.error("【Redis超时检测】取消超时订单失败: {}", e.getMessage());
            throw e;
        }
    }
    ...
    public void checkOrderTimeout() {
        ZSetOperations<String, String> zset = stringRedisTemplate.opsForZSet();
        distributedLockUtil.executeWithLock(OrderConstants.Cache.TIMEOUT_LOCK, () -> {
            Set<String> orderIds = zset.rangeByScore(OrderConstants.Cache.TIMEOUT_ZSET, 0, System.currentTimeMillis());
            if (orderIds != null && !orderIds.isEmpty()) {
                log.info("【Redis超时检测】发现超时未支付订单: {}", orderIds);
                orderAppService.cancelOrderTimeout(orderIds);
                zset.remove(OrderConstants.Cache.TIMEOUT_ZSET, orderIds.toArray());
            }
        });
