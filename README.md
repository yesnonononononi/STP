...
STP论坛是一个基于*Spring Cloud* 搭建的微服务项目,涉及数据库`34`张表,DDD领域架构,包含了用户认证,评论,优惠券,VIP系统,IM系统,订单系统,支付系统,帖子系统,用户系统以及排行榜系统等多个服务模块,提供游客模式和登录态,无需登录即可浏览网站帖子,评论等信息;登录用户可以在平台上自由发帖,评论,回复,私聊,关注,点赞,收藏,分享,个性化主页,进行每日签到以及充值VIP等操作
