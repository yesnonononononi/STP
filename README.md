...
- 由于用户认证模块的验证码服务需要对接第三方如`阿里云api`完成,本项目暂时并未开发用户认证服务中的注册验证码逻辑,需要自行补充(对接SDK并接入),同样的,设置模块中,邮箱绑定,手机号绑定也并未完成开发
...
由于本项目始终为本人+AI开发,对于功能测试较为少,一些`fallback`并未尝试`Test`,大部分功能处于正常可使用状态
...
要求: 运行机器配置有`nodeJs`,`jdk 8-21`,`nacos`,`mysql 8.0+`,`redis3+`,`rabbitMQ`,`minio service`

- 运行根目录的`.sql`后缀脚本,初始化数据库表(34张)
- 前端需要`npm/pnpm`安装项目技术栈所需组件,具体安装命令自查,完成后执行在`Stp_front`目录内执行`npm run dev`命令
- 后端在`Maven`自动下载完依赖后,确保运行机器配置符合要求且全部处于就绪状态后运行,在`STP`目录内执行`mvn intall`下载依赖并编译,确认编译完成无报错后,对每个服务执行`mvn spring-boot: run`命令运行
