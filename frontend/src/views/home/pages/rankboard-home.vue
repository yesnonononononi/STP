  ...
  <div class=" mb-2 self-stretch flex flex-col gap-4">
    ...
    <div
      ...
      <div class="flex flex-col gap-3 p-4 grow overflow-y-auto select-none">
        <!-- 话题热榜骨架屏 (Skeleton Loading) -->
        <template v-if="topicLoading">
          <div v-for="n in 6" :key="'topic-skel-' + n" class="flex items-center justify-between p-2 -mx-2 animate-pulse">
            <div class="flex items-center gap-3 w-full">
              <div class="w-5.5 h-5.5 rounded-md bg-gray-200 shrink-0"></div>
              <div class="h-4 bg-gray-200 rounded-md w-3/5"></div>
            </div>
            <div class="h-3.5 bg-gray-200 rounded-md w-10 shrink-0"></div>
          </div>
        </template>
    ...
    <div class="sticky rounded-xl top-17 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/70">
      <div class="rank flex flex-col relative h-160 bg-white rounded-xl">
        ...
        <div class="flex flex-col gap-3 p-4 grow overflow-y-auto select-none">
          <!-- 创作者热榜骨架屏 (Skeleton Loading) -->
          <template v-if="creatorLoading">
            <div v-for="n in 6" :key="'creator-skel-' + n" class="flex items-center justify-between p-2 -mx-2 animate-pulse">
              <div class="flex items-center gap-3 w-full">
                <div class="w-5.5 h-5.5 rounded-md bg-gray-200 shrink-0"></div>
                ...
                <div class="flex flex-col gap-1.5 w-1/2">
                  ...
                  <div class="h-2.5 bg-gray-200 rounded-md w-2/3"></div>
                </div>
              </div>
            </div>
          </template>
