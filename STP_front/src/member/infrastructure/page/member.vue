<template>
  <div class="w-260 h-150 border rounded-md z-10 p-6 relative" :style="{
    ...
    <div class="userInfo flex items-center h-12">
      <div class="avatar">
        <img class="w-12 h-12 rounded-full bg-gray-300" src="" alt="头像" />
      </div>
      <div class="description w-full h-12 m-2">
        ...
        <div class="ismember text-white text-sm">
          {{
            user?.isVip == 0
              ? '你还不是会员，开通立享特权'
              : `会员到期还有 ${TimeUtils.calculateTimeByNow(user?.endTime)} 天`
          }}
        </div>
      </div>
    ...
    <div class="main w-full h-120 p-2 bg-blue-100 shadow-md mt-4 rounded-md">
      ...
      <div class="avail_introduce w-full h-45 my-2">
...
import { Payer } from '@/payment/api/hooks/payer'
...
import { PayType } from '../../../payment/infrastructure/types/PayType'
import type { memberConfig } from '@/member/api/types/member'
import { PayFormBuilder } from '@/payment/infrastructure/utils/PayFormBuilder'
import type { payForm } from '@/domain/entities/pay'
import { log } from '@/infrastructure/Log'
import { MemberBase } from '@/member/api/hooks/base'
