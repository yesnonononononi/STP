<template>
  <div class="w-full animate-fade-in">
    <el-form ref="registerFormRef" :model="registerForm" :rules="registerRules" label-width="0" class="space-y-4">
      ...
      <div class="flex flex-col gap-4 mt-6">
        ...
        <div class="flex justify-center items-center text-sm mt-2">
          ...
          <router-link class="text-cyan-400 hover:text-cyan-300 font-medium transition-colors" to="/auth/login">立即登录</router-link>
        </div>
      </div>
    </el-form>
  </div>
...
import { Auther } from '../../api/hooks/Auth'
...
const sendVerifyCode = () => {
  ...
  // 模拟发送验证码，实际可调用API
  ElMessage.success('验证码已发送（测试模式：123456）')
...
const handleRegister = async () => {
  ...
  await registerFormRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        const res = await auther.register({
