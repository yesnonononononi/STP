<template>
    <div class="relative z-1 w-full min-h-screen bg-linear-to-br from-blue-50/70 via-indigo-50/50 to-purple-50/70 flex items-center justify-center p-4 md:p-6 overflow-hidden select-none">
        ...
        <!-- Main pay card -->
        <div class="w-full max-w-[480px] bg-white/85 backdrop-blur-xl border border-white/60 shadow-2xl rounded-3xl p-6 md:p-8 flex flex-col gap-6 transition-all duration-300 hover:shadow-indigo-150/40 relative z-10">
            <!-- Header -->
            <div class="flex items-center justify-between pb-2 border-b border-slate-100/80">
                <button @click="cancelPay" class="p-2 -ml-2 rounded-xl text-slate-400 hover:text-slate-650 hover:bg-slate-100/50 transition-all active:scale-95 cursor-pointer">
            ...
            <!-- Commodity Info Card -->
            <div class="bg-gradient-to-r from-slate-50 to-indigo-50/30 border border-slate-100 rounded-2xl p-4 flex items-center justify-between gap-4">
                <div class="flex gap-3 items-center">
                    <div class="relative flex items-center justify-center w-14 h-14 bg-amber-500/10 rounded-xl border border-amber-500/20 shadow-inner">
                        <!-- Shiny gold Jewel/Diamond SVG -->
                        <svg class="w-8 h-8 drop-shadow-[0_2px_8px_rgba(242,203,81,0.4)] animate-pulse" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex flex-col gap-1 justify-start">
                        <span class="font-bold text-slate-800 text-base leading-snug">{{ commodityInfo?.name || '未获取到商品信息' }}</span>
                        <span class="text-slate-400 text-xs font-semibold bg-slate-150/50 px-2 py-0.5 rounded-md w-max">数量：{{ quantity || 0 }}</span>
                    </div>
                </div>
                <div class="flex flex-col items-end gap-1.5">
                    <span class="text-xs text-slate-450 line-through">原价 ¥{{ commodityInfo?.price || '0.00' }}</span>
                    <div class="bg-amber-500/10 border border-amber-500/20 text-amber-700 text-[11px] font-bold px-2 py-0.5 rounded-full">
                        已节省约 ¥{{ discount }}
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-3.5 bg-slate-50/50 border border-slate-100 rounded-2xl p-4">
                ...
                <div class="flex items-center justify-between text-sm">
                    ...
                    <div class="cursor-pointer flex items-center gap-1.5 text-xs" @click="moreCoupon = true">
                        <span v-if="curCoupon" class="bg-rose-50 border border-rose-100 text-rose-600 font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-xs">
                            <span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping"></span>
                            -¥{{ discount }} ({{ curCoupon.name }})
                        </span>
                        <span v-else-if="canUseCoupon.length > 0" class="bg-amber-50 border border-amber-100 text-amber-750 font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1 animate-pulse">
            ...
            <div class="flex flex-col gap-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div v-for="(item, index) in payTypes" :key="index"
                        ...
                        <div class="absolute top-2.5 right-2.5 text-base"
                            :class="item === payType
                        ...
                        <div class="mb-2">
                            <!-- WeChat Pay Icon SVG -->
                            <svg class="w-10 h-10 transition-transform duration-300 group-hover:scale-105" viewBox="0 0 1228 1024" version="1.1"
                                v-if="item === PayType.WX_PAY" xmlns="http://www.w3.org/2000/svg">
                                <path
                            ...
                            <!-- Alipay Icon SVG -->
                            <svg class="w-10 h-10 transition-transform duration-300 group-hover:scale-105" viewBox="0 0 1024 1024" version="1.1"
                                v-if="item === PayType.ALI_PAY" xmlns="http://www.w3.org/2000/svg">
                                <path
            ...
            <div class="flex items-center gap-4 mt-4 pt-2">
                <button class="w-1/2 cursor-pointer border border-slate-200 text-slate-550 bg-white hover:bg-slate-50 hover:text-slate-800 p-3 rounded-2xl font-bold shadow-xs transition-all duration-200 active:scale-95 flex items-center justify-center gap-1.5"
                    @click="cancelPay">
                    取消订单
                </button>
                <button
    ...
    <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-45 transition-opacity duration-300 pointer-events-none"
        :class="moreCoupon ? 'opacity-100 pointer-events-auto' : 'opacity-0'"
        @click="moreCoupon = false"></div>

    <!-- Sidebar Coupon Drawer -->
    <div class="fixed inset-y-0 right-0 shadow-2xl w-full md:w-[400px] z-50 bg-white transition-all duration-500 overflow-y-auto"
        ...
        <div class="flex flex-col gap-6 p-6 h-full">
            <div class="flex flex-col gap-4 sticky top-0 bg-white z-30 pb-4 border-b border-slate-100">
                <div class="title flex items-center justify-between">
                    ...
                    <button class="w-8 h-8 rounded-full flex items-center justify-center bg-slate-50 text-slate-400 hover:text-slate-650 hover:bg-slate-100 transition-all active:scale-90 cursor-pointer" @click="moreCoupon = false">
                ...
                <div class="flex bg-slate-100 p-1 rounded-xl">
                    <button class="flex-1 py-2 text-xs font-bold rounded-lg cursor-pointer transition-all duration-200 flex items-center justify-center gap-1" 
            ...
            <div class="flex flex-col gap-4 overflow-y-auto grow pb-6">
                <!-- If empty -->
                <div v-if="(curCouponTab ? canUseCoupon : unUseCoupon).length === 0" class="flex flex-col items-center justify-center py-12 text-slate-400 gap-2">
                ...
                <div class="relative flex items-center h-28 rounded-2xl overflow-hidden shadow-sm transition-all duration-200 hover:scale-[1.01]"
                    :class="coupon.isAvailable 
                        ? 'bg-gradient-to-r from-amber-400 via-amber-550 to-orange-550 text-white cursor-pointer hover:shadow-md hover:shadow-amber-100/50' 
                    ...
                    <div class="absolute top-0 left-[30%] -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full z-20"></div>
                    <div class="absolute bottom-0 left-[30%] translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full z-20"></div>
                    ...
                    <div class="w-[70%] flex items-center justify-between p-4 h-full">
                        <div class="flex flex-col gap-1 justify-center text-xs grow pr-2">
                            <span class="font-bold text-sm leading-tight" :class="coupon.isAvailable ? 'text-white' : 'text-slate-650'">{{ coupon.name }}</span>
                            <span class="font-semibold text-[10px]" :class="coupon.isAvailable ? 'text-amber-150/90' : 'text-slate-400'">{{ substractTime(coupon.endTime || '') }} 后过期</span>
                            <!-- Ineligibility Reason -->
                            <span class="text-[10px] text-red-500 font-semibold mt-1 bg-red-50 border border-red-100/50 px-2 py-0.5 rounded-md w-max" v-if="!coupon.isAvailable">{{ coupon.reason || '当前商品不可用' }}</span>
                        </div>
                        ...
                        <div class="shrink-0 flex items-center" v-if="coupon.isAvailable">
                            <svg class="w-6 h-6 transition-all duration-200"
                                ...
                                <path
                                    d="M512 64C262.4 64 64 262.4 64 512s198.4 448 448 448 448-198.4 448-448S761.6 64 512 64z m236.8 326.4L454.4 684.8c-12.8 12.8-32 12.8-44.8 0L275.2 544c-12.8-12.8-12.8-32 0-44.8 12.8-12.8 32-12.8 44.8 0l115.2 115.2L704 339.2c12.8-12.8 32-12.8 44.8 0 12.8 12.8 12.8 38.4 0 51.2z"></path>
                            </svg>
                        </div>
                    </div>
                </div>
