  ...
  <div class="main relative flex min-h-screen flex-col items-center w-full">
    ...
    <div class="flex items-start w-full max-w-[1400px] px-4 gap-4 relative">
      ...
      <div class="right w-[30%] m-2 self-stretch flex flex-col gap-4">
        <!-- 话题热榜 -->
        <div class="rank flex flex-col relative h-160 rounded-xl bg-white border border-gray-100/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
          <div class="flex items-center h-16 w-full justify-between border-b border-b-gray-150/40 p-4">
            <span class="text-xl font-bold flex items-center before:content-[''] before:inline-block before:w-1 before:h-5 before:bg-linear-to-b before:from-rose-400 before:to-red-500 before:rounded-full before:mr-2">话题热榜</span>
            <span class="text-xs text-gray-400 cursor-pointer hover:text-red-500 transition-colors" @click="goToRankBoard('topic')">更多></span>
          </div>
          <div class="flex flex-col gap-3 p-4 grow overflow-y-auto select-none">
            <div class="flex items-center justify-between hover:bg-slate-50/60 p-2 -mx-2 rounded-lg transition-all duration-200 cursor-pointer" v-for="item in topicRankList" :key="item.id" @click="goToRankBoard('topic')">
              <div class="left flex-3 flex items-center gap-3 truncate">
                <!-- 序号徽章 -->
                <div class="w-6 h-6 flex items-center justify-center shrink-0">
                  <span v-if="item.sort === 1" class="bg-gradient-to-br from-rose-500 to-red-600 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(239,68,68,0.25)]">1</span>
                  <span v-else-if="item.sort === 2" class="bg-gradient-to-br from-slate-400 to-slate-500 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(100,116,139,0.25)]">2</span>
                  <span v-else-if="item.sort === 3" class="bg-gradient-to-br from-amber-400 to-amber-600 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(217,119,6,0.25)]">3</span>
                  <span v-else class="text-gray-400 font-semibold text-sm italic">{{ item.sort }}</span>
                </div>
                <div class="content text-sm text-wrap truncate text-gray-800 font-medium hover:text-blue-500 transition-colors">
                  #{{ item.title }}#
                </div>
              </div>
        ...
        <div class="sticky rounded-xl top-17 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/70">
          <div class="rank flex flex-col relative h-160 bg-white rounded-xl">
            <div class="flex items-center h-16 w-full justify-between border-b border-b-gray-150/40 p-4">
              <span class="text-xl font-bold flex items-center before:content-[''] before:inline-block before:w-1 before:h-5 before:bg-linear-to-b before:from-blue-400 before:to-indigo-500 before:rounded-full before:mr-2">创作者周榜</span>
              <span class="text-xs text-gray-400 cursor-pointer hover:text-blue-500 transition-colors" @click="goToRankBoard('creator')">更多></span>
            </div>
            <div class="flex flex-col gap-3 p-4 grow overflow-y-auto select-none">
              <div class="flex items-center justify-between hover:bg-slate-50/60 p-2 -mx-2 rounded-lg transition-all duration-200 cursor-pointer" v-for="ranker in creatorRankList" :key="ranker.id" @click="router.push({ name: 'userProfile', params: { id: ranker.id } })">
                <div class="left flex truncate items-center gap-3 max-w-[75%] min-w-0">
                  <!-- 序号徽章 -->
                  <div class="w-6 h-6 flex items-center justify-center shrink-0">
                    <span v-if="ranker.sort === 1" class="bg-gradient-to-br from-rose-500 to-red-600 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(239,68,68,0.25)]">1</span>
                    <span v-else-if="ranker.sort === 2" class="bg-gradient-to-br from-slate-400 to-slate-500 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(100,116,139,0.25)]">2</span>
                    <span v-else-if="ranker.sort === 3" class="bg-gradient-to-br from-amber-400 to-amber-600 text-white font-bold text-xs italic rounded-md size-5.5 flex items-center justify-center shadow-[0_2px_8px_rgba(217,119,6,0.25)]">3</span>
                    <span v-else class="text-gray-400 font-semibold text-sm italic">{{ ranker.sort }}</span>
                  </div>
                  <div class="relative w-10 h-10 shrink-0">
                    <img :src="ranker.avatar || ''" alt="avatar"
                      class="w-10 h-10 rounded-full border border-gray-100 object-cover bg-gray-100 shadow-xs">
