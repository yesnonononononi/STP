...
const handleForbiddenAdmin = (message: string) => {
...
request.interceptors.response.use(
  async (response: AxiosResponse<Result>) => {
    ...
    if (res?.code !== 1) {
      ...
      if (res?.code === 407) {
        return handleForbiddenAdmin(res?.errMsg || '权限不足，您无管理员访问权限')
      }
  ...
  async (error) => {
    ...
    if (response?.status === 407) {
      return handleForbiddenAdmin(response.data?.errMsg || '权限不足，您无管理员访问权限')
    }
    const message = response?.data?.errMsg || error?.message || '网络连接异常，请稍后再试'
    log.error(message)
