...
public class UserFollowController {
    ...
    @Login
    @GetMapping("/my-fans")
    @ApiOperation(value = "分页获取我的粉丝列表", notes = "分页获取当前登录用户的粉丝列表")
    public Result<Page<UserFansVO>> getMyFans(
            @ApiParam(value = "页码 (默认1)", required = false) @RequestParam(defaultValue = "1") long page,
            @ApiParam(value = "每页大小 (默认10)", required = false) @RequestParam(defaultValue = "10") long pageSize) {
        return Result.success(userFollowAppService.getMyFansPage(page, pageSize));
    }
}
