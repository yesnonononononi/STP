...
import com.summit.stp.comment.comment.application.service.AbstractCommentAppService;
...
public class CommentAppServiceImpl extends AbstractCommentAppService {
    ...
    public void deleteComment(Long id) {
        Comment comment = commentRepository.findById(id).orElseThrow(() -> new BusinessException("评论不存在"));
        if (!comment.canAccess())
            throw new BusinessException("评论不可达");
        List<CommentImage> images = commentImageRepository.findByCommentId(id);
    ...
    public boolean top(Long id, Long postId) {
        ...
        if (!(post.getCreatorId().equals(uid)))
